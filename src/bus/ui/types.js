export const SHOW_MENU_COLUMN = "showMenuColumn";
export const SHOW_FILTER = "showFilter";
export const SHOW_MODAL = "showModal";

export const HIDE_MENU_COLUMN = "hideMenuColumn";
export const HIDE_FILTER = "hideFilter";
export const HIDE_MODAL = "hideModal";

export const SORT_OR_FILTER = "sortOrFilter";
export const CHANGE_ORDER = "changeSortingOrder";
export const SET_ASC_ORDER = "setAscOrder";
export const SET_DESC_ORDER = "setDescOrder";
export const SET_SORTING_COLUMN = "setSortingColumn";
export const SET_FILTER_COLUMN = "setFilterColumn";
export const SET_FILTER_OPERATOR = "setFilterOperator";
export const SET_FILTER_VALUE = "setFilterValue";

export const SET_ROWS_AMOUNT = "setRowsAmount";
export const SET_PAGE_NUMBER = "setPageNumber";

export const SET_EDIT_PLACE = "setEditPlace";

export const SET_NEW_ROUTER_PAGE = "setNewRouterPage";
